`timescale 1ns/1ps
// ============================================================================
// dw_fused_axis.sv — AXIS shell around dw_fused_core (fused DW, stride 1 or 2)
// ============================================================================
//
...
//
// TLAST is OBSERVED, not predicted -- see the detailed section further down.
// (This header used to give the geometric formula n_groups*cin_run*n_rows*2;
// that scheme was replaced on 2026-07-25 and the formula is doubly stale now,
// being both 32b-era and stride-1-only. Nothing here needs updating when the
// emission count changes, which is exactly the point of the observed scheme:
// stride 2 quarters the output beat count and no constant moves.)
// ============================================================================
...
module dw_fused_axis #(
    ...
    // ---- AXIS slave (DW input from MM2S DMA, group-major, 64b = 1 group/beat) ----
    input  logic [63:0]             s_axis_tdata,
    input  logic                    s_axis_tvalid,
    output logic                    s_axis_tready,
    input  logic                    s_axis_tlast,   // unused

    // ---- AXIS master (to PW, group-major, 64b = 1 group/beat; matches PW's
    //      64b s_axis for a direct on-chip DW->PW cascade) ----
    output logic [63:0]             m_axis_tdata,
    output logic                    m_axis_tvalid,
    input  logic                    m_axis_tready,
    output logic                    m_axis_tlast,

    // ---- DEBUG (2026-07-25): observability for the hardware no-output hang ----
    output logic [31:0]             dbg_consumed,   // input words the core pulled
    output logic [31:0]             dbg_written,    // 64b groups written into out FIFO
    output logic [31:0]             dbg_produced,   // 32b beats handed to S2MM
    output logic [31:0]             dbg_win_state,  // windower FSM snapshot
    output logic [31:0]             dbg_flags,      // {..., all_written, core_done_seen}
    output logic [31:0]             dbg_win_cfg     // windower's latched {H_r,G_r,C_r}
);
    ...
    // ------------------------------------------------------------
    // Input FIFO: 64b write -> 64b read (one group per beat, no reassembly)
    // ------------------------------------------------------------
    ...
    // (A DWF_HALF_RATE_DIAG toggle briefly gated this signal on 2026-07-27 to
    // test whether the core was sensitive to the 64b build's doubled group
    // cadence. It was not -- the result was unchanged, because the throttle
    // stretched the pipeline in time but left the SEQUENCE of channel indices
    // identical, and the real fault was a per-channel ordering bug. Removed.)
    ...
    dw_fused_core #(.DATA_WIDTH(DATA_WIDTH), .ACC_WIDTH(ACC_WIDTH), .CIN_MAX(CIN_MAX)) u_core (
        ...
        .stride2(stride2), .zp_in(zp_in), .zp_out(zp_out), .relu_en(relu_en),
        ...
        .dbg_win_state(dbg_win_state), .dbg_win_cfg(dbg_win_cfg),
    ...
    // ------------------------------------------------------------
    // Output FIFO: 64b write -> 64b read (one group per beat, no serialization)
    // ------------------------------------------------------------
    logic              out_full, fifo_empty;
    logic [CORE_W-1:0] fifo_dout;
    ...
    //
    // BUG FIX 2026-07-28 -- USE_ADV_FEATURES was "0000", which DISABLES
    // prog_full. out_prog_full was therefore tied low forever and the
    // throttle above (core_valid_in = !in_empty && !out_prog_full) NEVER
    // ENGAGED; the threshold and this whole comment described a mechanism
    // that had never run. Under sustained backpressure the FIFO reached
    // full and out_wr_en = core_valid_out && !out_full silently DROPPED
    // beats. Harmless while the only consumer was a DMA that drained
    // faster than the core produced -- fatal once PW became the consumer,
    // because PW needs ~50 cycles per pixel group and backpressures hard.
    //
    // Measured on hardware (H=16 cascade): consumed=8640 written=4707
    // produced=4707, i.e. 3,933 of 8,640 output words destroyed, with
    // flags showing out_full_ever=1 AND out_prog_full=0 -- the FIFO hit
    // full while the "almost full" signal read zero. PW then starved
    // mid-run and hung forever waiting for words that no longer existed.
    // H=8 passed only because DW's 2048-deep out FIFO plus PW's 2048-deep
    // in FIFO happen to hold that run's entire 4,320-word output.
    //
    // "0002" enables prog_full and nothing else. Deliberately not "0707":
    // that would also enable the data-count ports, which are declared here
    // with WR/RD_DATA_COUNT_WIDTH(1) and would need widening to match the
    // FIFO depth.
    xpm_fifo_sync #(
        .DOUT_RESET_VALUE("0"), .ECC_MODE("no_ecc"), .FIFO_MEMORY_TYPE("block"),
        ...
        .RD_DATA_COUNT_WIDTH(1), .READ_DATA_WIDTH(CORE_W),
        .READ_MODE("fwft"), .SIM_ASSERT_CHK(0), .USE_ADV_FEATURES("0002"),
        .WAKEUP_TIME(0), .WRITE_DATA_WIDTH(CORE_W), .WR_DATA_COUNT_WIDTH(1)
    ) u_out_fifo (
    ...
    // ------------------------------------------------------------
    // DEBUG counters + flags (reset per run). Purely observational --
    // they tap existing signals and drive no functional logic.
    //   dbg_consumed : input words the core actually pulled (in_rd_en)
    //   dbg_written  : 64b groups the core pushed into the out FIFO (out_wr_en)
    //   dbg_produced : 32b beats accepted by S2MM (m_axis handshake)
    // dbg_written is THE fork: 0 => core emits nothing (windower dead);
    // >0 => core emits but the downstream hold/FIFO path isn't draining.
    // ------------------------------------------------------------
    logic        core_done_seen, out_full_ever;
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            dbg_consumed   <= 32'd0;
            dbg_written    <= 32'd0;
            ...
            if (in_rd_en)    dbg_consumed <= dbg_consumed + 32'd1;
            if (out_wr_en)   dbg_written  <= dbg_written  + 32'd1;
            if (m_axis_fire) dbg_produced <= dbg_produced + 32'd1;
            if (core_done)   core_done_seen <= 1'b1;
            if (out_full)    out_full_ever  <= 1'b1;
        end
    end

    assign dbg_flags = {26'd0, in_empty, out_prog_full, out_full_ever,
                        all_written, core_done_seen};
