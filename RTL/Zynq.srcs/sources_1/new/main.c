...
#define RUN_SURROGATE_ESTIMATE 0
 ...
 * ============================================================ */
static void pack_input_group_major(const uint8_t *src_unpadded, uint8_t *dst_group_major,
                                   int C, int H, int W, uint8_t pad_val)
{
    ...
    for (int h = 0; h < H; h++) {
        for (int g = 0; g < G_per_row; g++) {
            uint8_t *dst_group = dst_group_major + ((size_t)h * G_per_row + g) * (size_t)C * 8;
            int col0 = g * 8;
            for (int c = 0; c < C; c++) {
...
static int compare_group_major_vs_ref(const uint8_t *got_group_major, int C, int H, int W,
                                      ...
                                      const runner_cfg_t *cfg)
{
    static uint8_t cmp_unpack_scratch[256u * 1024u] __attribute__((aligned(64)));
    size_t need = tensor_bytes_u8(C, H, W);
    if (need > sizeof(cmp_unpack_scratch)) {
        printf("[CMP] %s group-major unpack scratch too small (%lu > %lu)\n",
               label, (unsigned long)need, (unsigned long)sizeof(cmp_unpack_scratch));
        return -1;
    }
    unpack_group_major_to_channel_major(got_group_major, cmp_unpack_scratch, C, H, W);
    return compare_tensor_vs_ref(cmp_unpack_scratch, C, H, W, ref_path, label, cfg);
}
                                        ...
                                        const runner_cfg_t *cfg)
{
    ...
    t = 0;
    while (XAxiDma_Busy(&DwDma, XAXIDMA_DEVICE_TO_DMA)) {
        if (++t > 50000000) { printf("[DWF-L0] S2MM timeout\n"); return -1; }
    }
    t = 0;
...
{
    static const int          cins [SURR_NUM_LAYERS] = {  3,  3,  8,  8, 16, 16 };
    static const int          couts[SURR_NUM_LAYERS] = {  3,  8,  8, 16, 16, 64 };
    static const layer_kind_t kinds[SURR_NUM_LAYERS] = {
        KIND_DW, KIND_PW, KIND_DW, KIND_PW, KIND_DW, KIND_PW
    };
    ...
    for (int i = 0; i < SURR_NUM_LAYERS; i++) {
        descs[i].layer_idx   = i;
        descs[i].kind        = kinds[i];
        descs[i].Cin         = cins[i];
        descs[i].Cout        = couts[i];
        descs[i].H           = H;
        descs[i].W           = W;
        descs[i].zp_in       = 128;
        descs[i].zp_out      = 128;
        descs[i].relu_en     = 0;
        descs[i].weight_addr = 0;
        descs[i].params_addr = 0;

        if (kinds[i] == KIND_DW) {
            /* Carries the stage's stride-2 downsampling. */
            descs[i].stride = 2;
            descs[i].pad    = 1;
            descs[i].kernel = 3;
            H = calc_out_dim(H, descs[i].pad, descs[i].kernel, descs[i].stride);
            W = calc_out_dim(W, descs[i].pad, descs[i].kernel, descs[i].stride);
        } else {
            /* 1x1/stride-1 -- spatial size unchanged by the PW half. */
            descs[i].stride = 1;
            descs[i].pad    = 0;
            descs[i].kernel = 1;
        }
...
int cnn_run_dw_pw_surrogate(void)
{
    ...
    for (int i = 0; i < SURR_NUM_LAYERS; i++) {
        ...
        if (d->kind == KIND_DW) {
            rc = run_dw_layer_real(d, buf_cur, buf_next, &p_blob[i], &w_blob[i]);
        } else {
...
#if RUN_SURROGATE_ESTIMATE
    rc = cnn_run_dw_pw_surrogate();
#else
