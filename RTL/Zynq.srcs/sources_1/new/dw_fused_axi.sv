...
//                      stride2 (NEW 2026-07-30) rides in this register rather
//                      than taking a new address so the IP's AXI-Lite address
//                      map -- and component.xml -- are unchanged. It is as
//                      quasi-static as the fields beside it, so the existing
//                      dw_fused_timing.xdc multicycle on reg_zp_relu_reg[*]
//                      covers it correctly.
//                      PRECONDITIONS when stride2=1 (driver must enforce;
//                      hardware does not check): N_GROUPS even (IMG_WIDTH a
//                      multiple of 16) and CIN_RUN >= 2. See the SCOPE block
//                      in dw_banked_window_8x.sv.
...
module dw_fused_axi #(
    ...
    // AXI-Stream input (from MM2S DMA, group-major, 64b = 1 group/beat)
    input  logic [63:0]                       s_axis_tdata,
    input  logic                              s_axis_tvalid,
    output logic                              s_axis_tready,
    input  logic                              s_axis_tlast,

    // AXI-Stream output (to PW, group-major, 64b = 1 group/beat; matches PW's
    // 64b s_axis for the direct on-chip DW->PW cascade)
    output logic [63:0]                       m_axis_tdata,
    output logic                              m_axis_tvalid,
    input  logic                              m_axis_tready,
    output logic                              m_axis_tlast,

    // Per-run start pulse, exposed 2026-07-25 so downstream BD blocks (namely
    // dw_reorder_0) can give themselves a fresh per-run reset instead of only
    // resetting once at power-on -- see dw_reorder.sv's header comment for
    // why that mattered (a stuck FIFO from an aborted run can silently block
    // or corrupt the next run's data, including its TLAST beat).
    ...
    // DEBUG read-only registers (2026-07-25), for the hardware no-output hang.
    localparam logic [11:0] ADDR_DBG_CONSUMED = 12'h038;
    localparam logic [11:0] ADDR_DBG_WRITTEN  = 12'h03C;
    localparam logic [11:0] ADDR_DBG_PRODUCED = 12'h040;
    localparam logic [11:0] ADDR_DBG_WINSTATE = 12'h044;
    localparam logic [11:0] ADDR_DBG_FLAGS    = 12'h048;
    localparam logic [11:0] ADDR_DBG_WINCFG   = 12'h04C;
    ...
    // DEBUG signals from the datapath (read back via AXI-Lite after a hang).
    logic [31:0] dbg_consumed, dbg_written, dbg_produced, dbg_win_state, dbg_flags, dbg_win_cfg;

    assign start_pulse_out = start_pulse;
    ...
    always_comb begin
        ...
        case (rd_addr)
            ...
            ADDR_DBG_CONSUMED : s_axi_rdata = dbg_consumed;
            ADDR_DBG_WRITTEN  : s_axi_rdata = dbg_written;
            ADDR_DBG_PRODUCED : s_axi_rdata = dbg_produced;
            ADDR_DBG_WINSTATE : s_axi_rdata = dbg_win_state;
            ADDR_DBG_FLAGS    : s_axi_rdata = dbg_flags;
            ADDR_DBG_WINCFG   : s_axi_rdata = dbg_win_cfg;
    ...
    dw_fused_axis #(
        ...
        .dbg_consumed(dbg_consumed), .dbg_written(dbg_written),
        .dbg_produced(dbg_produced), .dbg_win_state(dbg_win_state),
        .dbg_flags(dbg_flags), .dbg_win_cfg(dbg_win_cfg)
    );
