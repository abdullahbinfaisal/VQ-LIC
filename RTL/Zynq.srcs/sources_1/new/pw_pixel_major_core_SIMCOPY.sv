...
module pw_pixel_major_core_SIMCOPY #(
  ...
  always_comb begin
    ...
    for (int oc = 0; oc < N_OC; oc++) begin
      for (int p = 0; p < N_LANES/2; p++) begin
        p_packed_comb[oc][p] = a_packed[p] * w_rd_data_r[oc];
      end
    end
  end
    ...
    if (!rst_n) begin
      ...
      case (st)
        ...
        S_COMPUTE: begin
          ...
          // Pipeline stage 3: calculate product AND register it
          // Dual-MAC packed product computed combinatorially above is registered
          // directly here to map optimally to the DSP48 MREG.
          if (rd_issued_d1) begin
            mul_valid <= 1'b1;
          end else begin
            mul_valid <= 1'b0;
          ...
          if ($unsigned(ic_idx) < cin_run) begin
            ...
            rd_issued <= 1'b0;
            if (!rd_issued && !rd_issued_d1 && !mul_valid) begin
  ...
  // ------------------------------------------------------------
  // Simulation Safeguards & Assertions
  // ------------------------------------------------------------
  // pragma translate_off
  // 1. Runtime Parameter Validation Assertions
  always @(posedge clk) begin
    if (start_in) begin
      if (cin_run > CIN_MAX) begin
        $error("SIM_ERROR: cin_run (%0d) exceeds CIN_MAX (%0d)", cin_run, CIN_MAX);
      end
      if (cout_run > COUT_MAX) begin
        $error("SIM_ERROR: cout_run (%0d) exceeds COUT_MAX (%0d)", cout_run, COUT_MAX);
      end
      if (cout_run % N_OC != 0) begin
        $error("SIM_ERROR: cout_run (%0d) must be a multiple of N_OC (%0d)", cout_run, N_OC);
      end
      if (tile_pixels % N_LANES != 0) begin
        $error("SIM_ERROR: tile_pixels (%0d) must be a multiple of N_LANES (%0d)", tile_pixels, N_LANES);
      end
    end
  end

  // 2. Shadow BRAM Collision Safeguard
  always @(posedge clk) begin
    if (sha_wr_en && sha_rd_en && (sha_wr_addr == sha_rd_addr)) begin
      $error("SIM_ERROR: Shadow BRAM Collision: Simultaneous read and write access to same address %0d!", sha_wr_addr);
    end
  end

  // 3. Output Buffer Overflow Hazard Check
  always @(posedge clk) begin
    if (valid_out && out_stall) begin
      $warning("SIM_WARNING: Core outputted data while out_stall is active. If backpressure has no headroom, this will cause data loss!");
    end
  end
  // pragma translate_on
