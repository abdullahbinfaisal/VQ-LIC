...
__attribute__((always_inline)) static inline uint8_t clamp_u8_i32(int32_t x)
{
    if (x < 0) x = 0;
    if (x > 255) x = 255;
    return (uint8_t)x;
...
#if USE_L1_SPLIT_LUT
__attribute__((unused)) static void build_residual_add_lut_split_exact(const add_desc_t *ad)
{
    for (int i = 0; i < 256; i++) {
...
__attribute__((always_inline, target("fpu=neon"))) static inline int32x2_t neon_round_shift_ties_to_even_const30(int64x2_t acc, int32x2_t zp_out)
{
    int64x2_t sign_mask = vshrq_n_s64(acc, 63);

    int64x2_t ax = vsubq_s64(vreinterpretq_s64_u64(veorq_u64(
        vreinterpretq_u64_s64(acc), vreinterpretq_u64_s64(sign_mask))), sign_mask);

    int32x2_t ax_32 = vmovn_s64(ax);
    int32x2_t q_32 = vmovn_s64(vshrq_n_s64(ax, 30));
    int32x2_t r_32 = vand_s32(ax_32, vdup_n_s32(0x3FFFFFFF));
    int32x2_t half_32 = vdup_n_s32(0x20000000);

    uint32x2_t gt_mask = vcgt_s32(r_32, half_32);
    uint32x2_t eq_mask = vceq_s32(r_32, half_32);
    uint32x2_t odd_mask = vtst_s32(q_32, vdup_n_s32(1));
    uint32x2_t inc_mask = vorr_u32(gt_mask, vand_u32(eq_mask, odd_mask));

    q_32 = vsub_s32(q_32, vreinterpret_s32_u32(inc_mask));

    int32x2_t sign_mask_32 = vmovn_s64(sign_mask);
    int32x2_t y_32 = vsub_s32(veor_s32(q_32, sign_mask_32), sign_mask_32);

    return vadd_s32(y_32, zp_out);
}

__attribute__((always_inline, target("fpu=neon"))) static inline int32x2_t neon_round_shift_ties_to_even_const31(int64x2_t acc, int32x2_t zp_out)
{
    int64x2_t sign_mask = vshrq_n_s64(acc, 63);
    ...
    int32x2_t ax_32 = vmovn_s64(ax);
    int32x2_t q_32 = vmovn_s64(vshrq_n_s64(ax, 31));
    int32x2_t r_32 = vand_s32(ax_32, vdup_n_s32(0x7FFFFFFF));
    int32x2_t half_32 = vdup_n_s32(0x40000000);
...
__attribute__((always_inline)) static inline int32_t round_shift_ties_to_even_i64_const30(int64_t x)
{
    int sign = (x < 0);
    uint64_t ax = sign ? (uint64_t)(-x) : (uint64_t)x;

    uint32_t q32 = (uint32_t)(ax >> 30);
    uint32_t r32 = (uint32_t)ax & 0x3FFFFFFFu;
    uint32_t half32 = 0x20000000u;

    int inc = (r32 > half32) | ((r32 == half32) & (q32 & 1u));
    q32 += inc;

    return sign ? -(int32_t)q32 : (int32_t)q32;
}

__attribute__((always_inline)) static inline int32_t round_shift_ties_to_even_i64_const31(int64_t x)
{
    int sign = (x < 0);
    uint64_t ax = sign ? (uint64_t)(-x) : (uint64_t)x;

    uint32_t q32 = (uint32_t)(ax >> 31);
    uint32_t r32 = (uint32_t)ax & 0x7FFFFFFFu;
    uint32_t half32 = 0x40000000u;
...
__attribute__((target("fpu=neon"))) void run_residual_add_tensor(const uint8_t * restrict src_a,
                             ...
                             const add_desc_t *ad)
{
    ...
    int32x2_t zp_a_v = vdup_n_s32(ad->zp_a);
    int32x2_t zp_b_v = vdup_n_s32(ad->zp_b);
    int32x2_t zp_out_v = vdup_n_s32(zp_out);
    int32_t mult_a = (int32_t)ad->mult_a;
    int32_t mult_b = (int32_t)ad->mult_b;

    int corr_a = (ad->mult_a & 0x80000000u) != 0;
    int corr_b = (ad->mult_b & 0x80000000u) != 0;

    size_t i = 0;
    if (shift == 30 || shift == 31) {
        for (; i + 4 <= n_elem; i += 4) {
            uint8x8_t a_u8 = vld1_u8(a_ptr + i);
            uint8x8_t b_u8 = vld1_u8(b_ptr + i);

            int16x4_t a_s16 = vreinterpret_s16_u16(vget_low_u16(vmovl_u8(a_u8)));
            int16x4_t b_s16 = vreinterpret_s16_u16(vget_low_u16(vmovl_u8(b_u8)));
            int32x4_t a_s32 = vmovl_s16(a_s16);
            int32x4_t b_s32 = vmovl_s16(b_s16);

            int32x2_t da_l = vsub_s32(vget_low_s32(a_s32), zp_a_v);
            int32x2_t da_h = vsub_s32(vget_high_s32(a_s32), zp_a_v);
            int32x2_t db_l = vsub_s32(vget_low_s32(b_s32), zp_b_v);
            int32x2_t db_h = vsub_s32(vget_high_s32(b_s32), zp_b_v);

            int64x2_t prod_a_l = vmull_n_s32(da_l, mult_a);
            int64x2_t prod_a_h = vmull_n_s32(da_h, mult_a);
            if (corr_a) {
                prod_a_l = vaddq_s64(prod_a_l, vshll_n_s32(da_l, 32));
                prod_a_h = vaddq_s64(prod_a_h, vshll_n_s32(da_h, 32));
            }

            int64x2_t prod_b_l = vmull_n_s32(db_l, mult_b);
            int64x2_t prod_b_h = vmull_n_s32(db_h, mult_b);
            if (corr_b) {
                prod_b_l = vaddq_s64(prod_b_l, vshll_n_s32(db_l, 32));
                prod_b_h = vaddq_s64(prod_b_h, vshll_n_s32(db_h, 32));
            }

            int64x2_t acc_l = vaddq_s64(prod_a_l, prod_b_l);
            int64x2_t acc_h = vaddq_s64(prod_a_h, prod_b_h);

            int32x2_t res_l, res_h;
            if (shift == 30) {
                res_l = neon_round_shift_ties_to_even_const30(acc_l, zp_out_v);
                res_h = neon_round_shift_ties_to_even_const30(acc_h, zp_out_v);
            } else {
                res_l = neon_round_shift_ties_to_even_const31(acc_l, zp_out_v);
                res_h = neon_round_shift_ties_to_even_const31(acc_h, zp_out_v);
            }

            int32x4_t res_s32 = vcombine_s32(res_l, res_h);
            int32x4_t clamped = vmaxq_s32(vminq_s32(res_s32, vdupq_n_s32(255)), vdupq_n_s32(0));

            int16x4_t res_s16 = vmovn_s32(clamped);
            uint8x8_t res_u8 = vmovn_u16(vreinterpretq_u16_s16(vcombine_s16(res_s16, vdup_n_s16(0))));

            vst1_lane_u32((uint32_t *)(d_ptr + i), vreinterpret_u32_u8(res_u8), 0);
        }
    }

    // Scalar tail (handles remainder and any shift not == 30 or 31)
    if (shift == 30) {
            uint8x8_t a_u8 = vld1_u8(a_ptr + i);
            uint8x8_t b_u8 = vld1_u8(b_ptr + i);
            ...
            int16x4_t a_s16_l = vreinterpret_s16_u16(vget_low_u16(a_u16));
            int16x4_t b_s16_l = vreinterpret_s16_u16(vget_low_u16(b_u16));

            // Low 4 elements
            int32x4_t a_s32_l = vmovl_s16(a_s16_l);
            int32x4_t b_s32_l = vmovl_s16(b_s16_l);

            int32x2_t da_l_l = vsub_s32(vget_low_s32(a_s32_l), zp_a_v);
            int32x2_t da_h_l = vsub_s32(vget_high_s32(a_s32_l), zp_a_v);
            int32x2_t db_l_l = vsub_s32(vget_low_s32(b_s32_l), zp_b_v);
            int32x2_t db_h_l = vsub_s32(vget_high_s32(b_s32_l), zp_b_v);

            int64x2_t prod_a_l_l = vmull_n_s32(da_l_l, mult_a);
            int64x2_t prod_a_h_l = vmull_n_s32(da_h_l, mult_a);
            if (corr_a) {
                prod_a_l_l = vaddq_s64(prod_a_l_l, vshll_n_s32(da_l_l, 32));
                prod_a_h_l = vaddq_s64(prod_a_h_l, vshll_n_s32(da_h_l, 32));
            }

            int64x2_t prod_b_l_l = vmull_n_s32(db_l_l, mult_b);
            int64x2_t prod_b_h_l = vmull_n_s32(db_h_l, mult_b);
            if (corr_b) {
                prod_b_l_l = vaddq_s64(prod_b_l_l, vshll_n_s32(db_l_l, 32));
                prod_b_h_l = vaddq_s64(prod_b_h_l, vshll_n_s32(db_h_l, 32));
            }

            int64x2_t acc_l_l = vaddq_s64(prod_a_l_l, prod_b_l_l);
            int64x2_t acc_h_l = vaddq_s64(prod_a_h_l, prod_b_h_l);

            int32x2_t res_l_l = neon_round_shift_ties_to_even_const30(acc_l_l, zp_out_v);
            int32x2_t res_h_l = neon_round_shift_ties_to_even_const30(acc_h_l, zp_out_v);

            int32x4_t res_s32_l = vcombine_s32(res_l_l, res_h_l);
            int32x4_t clamped_l = vmaxq_s32(vminq_s32(res_s32_l, clamp_max), clamp_min);
            int16x4_t res_s16_l = vmovn_s32(clamped_l);

            // High 4 elements
            int16x4_t a_s16_h = vreinterpret_s16_u16(vget_high_u16(a_u16));
            int16x4_t b_s16_h = vreinterpret_s16_u16(vget_high_u16(b_u16));

            int32x4_t a_s32_h = vmovl_s16(a_s16_h);
            int32x4_t b_s32_h = vmovl_s16(b_s16_h);

            int32x2_t da_l_h = vsub_s32(vget_low_s32(a_s32_h), zp_a_v);
            int32x2_t da_h_h = vsub_s32(vget_high_s32(a_s32_h), zp_a_v);
            int32x2_t db_l_h = vsub_s32(vget_low_s32(b_s32_h), zp_b_v);
            int32x2_t db_h_h = vsub_s32(vget_high_s32(b_s32_h), zp_b_v);

            int64x2_t prod_a_l_h = vmull_n_s32(da_l_h, mult_a);
            int64x2_t prod_a_h_h = vmull_n_s32(da_h_h, mult_a);
            if (corr_a) {
                prod_a_l_h = vaddq_s64(prod_a_l_h, vshll_n_s32(da_l_h, 32));
                prod_a_h_h = vaddq_s64(prod_a_h_h, vshll_n_s32(da_h_h, 32));
            }

            int64x2_t prod_b_l_h = vmull_n_s32(db_l_h, mult_b);
            int64x2_t prod_b_h_h = vmull_n_s32(db_h_h, mult_b);
            if (corr_b) {
                prod_b_l_h = vaddq_s64(prod_b_l_h, vshll_n_s32(db_l_h, 32));
                prod_b_h_h = vaddq_s64(prod_b_h_h, vshll_n_s32(db_h_h, 32));
            }

            int64x2_t acc_l_h = vaddq_s64(prod_a_l_h, prod_b_l_h);
            int64x2_t acc_h_h = vaddq_s64(prod_a_h_h, prod_b_h_h);

            int32x2_t res_l_h = neon_round_shift_ties_to_even_const30(acc_l_h, zp_out_v);
            int32x2_t res_h_h = neon_round_shift_ties_to_even_const30(acc_h_h, zp_out_v);

            int32x4_t res_s32_h = vcombine_s32(res_l_h, res_h_h);
            int32x4_t clamped_h = vmaxq_s32(vminq_s32(res_s32_h, clamp_max), clamp_min);
            int16x4_t res_s16_h = vmovn_s32(clamped_h);

            // Combine and store 8 bytes
            uint8x8_t res_u8 = vmovn_u16(vreinterpretq_u16_s16(vcombine_s16(res_s16_l, res_s16_h)));
            vst1_u8(d_ptr + i, res_u8);
        }
    } else if (shift == 31) {
        int32x4_t clamp_max = vdupq_n_s32(255);
        int32x4_t clamp_min = vdupq_n_s32(0);
        for (; i + 8 <= n_elem; i += 8) {
            uint8x8_t a_u8 = vld1_u8(a_ptr + i);
            uint8x8_t b_u8 = vld1_u8(b_ptr + i);
            ...
            int32x2_t res_l_l = neon_round_shift_ties_to_even_const31(acc_l_l, zp_out_v);
            int32x2_t res_h_l = neon_round_shift_ties_to_even_const31(acc_h_l, zp_out_v);
            ...
            int32x2_t res_l_h = neon_round_shift_ties_to_even_const31(acc_l_h, zp_out_v);
            int32x2_t res_h_h = neon_round_shift_ties_to_even_const31(acc_h_h, zp_out_v);
    ...
    if (shift == 30) {
        for (; i < n_elem; i++) {
            int64_t da = (int64_t)a_ptr[i] - ad->zp_a;
            int64_t db = (int64_t)b_ptr[i] - ad->zp_b;
            int64_t acc = da * ad->mult_a + db * ad->mult_b;
            d_ptr[i] = clamp_u8_i32(round_shift_ties_to_even_i64_const30(acc) + zp_out);
        }
    } else if (shift == 31) {
        for (; i < n_elem; i++) {
            int64_t da = (int64_t)a_ptr[i] - ad->zp_a;
            int64_t db = (int64_t)b_ptr[i] - ad->zp_b;
            int64_t acc = da * ad->mult_a + db * ad->mult_b;
            d_ptr[i] = clamp_u8_i32(round_shift_ties_to_even_i64_const31(acc) + zp_out);
        }
    } else {
        for (; i < n_elem; i++) {
            int64_t da = (int64_t)a_ptr[i] - ad->zp_a;
            int64_t db = (int64_t)b_ptr[i] - ad->zp_b;
            int64_t acc = da * ad->mult_a + db * ad->mult_b;
            d_ptr[i] = clamp_u8_i32(round_shift_ties_to_even_i64_fast(acc, shift) + zp_out);
        }
    }
    ...
    printf("[ADD TIMING] lut mode   : NEON vectorized exact\n");
