...
module dw_fused_axis #(
    ...
    // ------------------------------------------------------------
    // TLAST / done: total 32b output beats = n_groups * cin_run * n_rows * 2.
    // Three-way product, so it's pipelined over 2 registered-multiply stages
    // (mirrors dw_plane_run_axis.sv's out_words_r pattern) rather than one
    // long combinational 12x12x12 chain. mult_v isn't valid until 2 cycles
    // after start_in; safe, since the core's own datapath latency (>=18
    // cycles) means no real output beat can appear anywhere near that soon.
    // ------------------------------------------------------------
    logic [23:0] gc_prod_r;       // n_groups * cin_run, stage 1
    logic [11:0] n_rows_r;
    logic        mult_stage2;
    logic [31:0] total_words_r;   // gc_prod_r * n_rows_r, stage 2
    logic        mult_v;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            gc_prod_r   <= n_groups * cin_run;   // 12x12 -> 24b
            n_rows_r    <= n_rows;
            mult_stage2 <= 1'b1;
            mult_v      <= 1'b0;
        end else if (mult_stage2) begin
            total_words_r <= gc_prod_r * n_rows_r;   // 24x12 -> up to 36b, fits 32b for real layers
            mult_stage2   <= 1'b0;
            mult_v        <= 1'b1;
        end
    end
    wire [32:0] total_beats = {total_words_r, 1'b0};   // *2 (two 32b beats / group)

    logic [32:0] produced_cnt;
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n)        produced_cnt <= 33'd0;
        else if (start_in) produced_cnt <= 33'd0;
        else if (out_rd_en) produced_cnt <= produced_cnt + 33'd1;
    end
    assign m_axis_tlast = m_axis_tvalid && mult_v && (total_beats != 0) &&
                          (produced_cnt == total_beats - 33'd1);
