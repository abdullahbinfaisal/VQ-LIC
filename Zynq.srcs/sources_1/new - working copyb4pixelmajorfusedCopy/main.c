...
#define MAX_TENSOR_BYTES   176332800u
...
{
    return (size_t)C * (size_t)H * (size_t)W;
}
 ...
 * Residual add (CPU)
 * ============================================================ */
static int32_t round_shift_ties_to_even_i64(int64_t x, uint8_t shift)
{
    int sh = (int)shift;
    int sign;
                           ...
                           int zp_a, int zp_b, int zp_out)
{
    ...
    int64_t acc = (int64_t)da * (int64_t)mult_a + (int64_t)db * (int64_t)mult_b;
                             ...
                             const add_desc_t *ad)
{
    for (size_t i = 0; i < n_elem; i++) {
        dst[i] = residual_add_pixel((int)src_a[i], (int)src_b[i],
                                    ad->mult_a, ad->mult_b, ad->shift,
                                    ad->zp_a, ad->zp_b, ad->zp_out);
    }
}
 ...
 * ============================================================ */
int compare_tensor_vs_ref(const uint8_t *got, size_t n,
                          ...
                          const runner_cfg_t *cfg)
{
    ...
    if (load_file_from_sd(ref_path, &ref) != 0) {
        printf("[CMP] %s cannot open ref %s\n", label, ref_path);
        return -1;
    }

    if (ref.size != n) {
        printf("[CMP] %s size mismatch got=%lu ref=%lu\n",
               label, (unsigned long)n, (unsigned long)ref.size);
        free_blob(&ref);
        return -1;
    }
    ...
    for (size_t i = 0; i < n; i++) {
        if (got[i] != ref.data[i]) {
            if (mismatches < 8) {
                printf("[CMP] %s idx=%lu got=%u ref=%u\n",
                       label, (unsigned long)i, got[i], ref.data[i]);
            }
    ...
    if (mismatches == 0) printf("[CMP] %s PASS\n", label);
    else printf("[CMP] %s FAIL mismatches=%d first=%d\n", label, mismatches, first_idx);
...
static int hw_dw_run_plane(const uint8_t *in_plane,
                           ...
                           const int8_t w9[9])
{
    const int in_pixels  = H * W;
    const int out_h      = (H + 2 * pad - 3) / stride + 1;
    const int out_w      = (W + 2 * pad - 3) / stride + 1;
...
static int run_pw_once(int tile_pixels, int cin_run,
                       ...
                       uint8_t *rx_buf, int rx_len)
{
    ...
    printf("[PW_ONCE] tile_pixels=%d cin_run=%d tx_len=%d rx_len=%d tx=0x%08X rx=0x%08X\n",
           tile_pixels, cin_run, tx_len, rx_len,
    ...
    printf("[PW_ONCE] readback: TILE_PIXELS=%u CIN_RUN=%u ZP_RELU=0x%08X\n",
                                ...
                                uint32_t            weight_addr)
{
    ...
    /* ------------------------------------------------------------
     * Step 2: prepare tile metadata / packed input
     * ------------------------------------------------------------ */
    if (single_tile) {
        tile_offsets[0] = 0;
        ...
        for (int pix_base = 0; pix_base < total_pixels; pix_base += PW_TILE_PIXELS) {
            ...
            tile_offsets[ntiles] = (size_t)pix_base; /* stores pix_base in Case B */
            ntiles++;
        }
    }
     ...
     * ------------------------------------------------------------ */
    if (single_tile) {
        for (int oc = 0; oc < Cout; oc += PW_N_OC) {
            int run_ocs = ((oc + PW_N_OC) <= Cout) ? PW_N_OC : (Cout - oc);

            {
                u64_cycles t0 = timer_now();
                for (int bank = 0; bank < run_ocs; bank++) {
                    int curr_oc = oc + bank;
                    if (load_pw_weights_for_oc(weights_blob, weight_addr, curr_oc, Cin, w_ic) != 0) {
                        printf("[PW] weight load fail oc=%d\n", curr_oc);
                        return -1;
                    }
                    
                    pw_write_reg(PW_REG_OC_SEL, bank);
                    pw_write_reg(PW_REG_BIAS,  (u32)bias_cache[curr_oc]);
                    pw_write_reg(PW_REG_MULT,  mult_cache[curr_oc]);
                    pw_write_reg(PW_REG_SHIFT, shift_cache[curr_oc]);
            ...
            {
                u64_cycles t0 = timer_now();
                const int rx_len = total_pixels * run_ocs;

                if (run_pw_once(total_pixels, Cin,
                                zp_in, zp_out, relu_en,
                                (uint8_t *)in_tensor, total_pixels * Cin,
                                pw_rx_tile, rx_len) != 0) {
                    printf("[PW] run fail oc=%d single_tile\n", oc);
                    return -1;
                }
...

            {
                u64_cycles t0 = timer_now();
                for (int bank = 0; bank < run_ocs; bank++) {
                    memcpy(out_tensor + ((size_t)(oc + bank) * (size_t)total_pixels),
                           pw_rx_tile + (bank * total_pixels),
                           (size_t)total_pixels);
                }
                cyc_store += timer_now() - t0;
            }

            if ((oc % 16) == 0 || oc >= Cout - PW_N_OC) {
                printf("[PW] single-tile completed oc %d / %d\n", oc, Cout);
            }
        }

    /* ------------------------------------------------------------
     * Step 3B: full-pack multi-tile
     * ------------------------------------------------------------ */
    } else if (use_full_pack) {
        for (int oc = 0; oc < Cout; oc += PW_N_OC) {
            int run_ocs = ((oc + PW_N_OC) <= Cout) ? PW_N_OC : (Cout - oc);
            ...
            for (int t = 0; t < ntiles; t++) {
                ...
                const int rx_len      = tile_pixels * run_ocs;
                const int pix_base    = t * PW_TILE_PIXELS;
                const uint8_t *tx_ptr = full_pack_buf + tile_offsets[t];

                {
                    u64_cycles t0 = timer_now();

                    if (run_pw_once(tile_pixels, Cin,
                                    zp_in, zp_out, relu_en,
                                    (uint8_t *)tx_ptr, tx_len,
                                    pw_rx_tile, rx_len) != 0) {
                        printf("[PW] run fail oc=%d tile=%d\n", oc, t);
                        return -1;
                    }
...

                {
                    u64_cycles t0 = timer_now();
                    for (int bank = 0; bank < run_ocs; bank++) {
                        memcpy(out_tensor + ((size_t)(oc + bank) * (size_t)total_pixels) + (size_t)pix_base,
                               pw_rx_tile + (bank * tile_pixels),
                               (size_t)tile_pixels);
                    }
                    cyc_store += timer_now() - t0;
                }
    ...
    /* ------------------------------------------------------------
     * Step 3C: scratch-tile multi-tile
     * Pack each tile once, then run all OCs on it
     * ------------------------------------------------------------ */
    } else {
        for (int t = 0; t < ntiles; t++) {
            const int tile_pixels = tile_sizes[t];
            const int pix_base    = (int)tile_offsets[t];
            const int tx_len      = tile_pixels * Cin;
            ...
            if ((t % 4) == 0 || t == ntiles - 1) {
                printf("[PW] scratch path packed tile %d / %d\n", t + 1, ntiles);
            }
        }
    }
...

    {
        ...
        const int repack_count_orig   = single_tile ? 0 : (Cout * ntiles);
        const int repack_count_actual = single_tile ? 0 : (use_full_pack ? 1 : ntiles);

        printf("[PW TIMING] H=%d W=%d Cin=%d Cout=%d total_pixels=%d ntiles=%d case=%s\n",
               H, W, Cin, Cout, total_pixels, ntiles, case_str);

        if (!single_tile) {
            printf("[PW TIMING] repack calls: original=%d actual=%d reduction=%.1fx\n",
                   repack_count_orig,
                   repack_count_actual,
                   (double)repack_count_orig / (double)repack_count_actual);
        }

        printf("[PW TIMING] total   : %.3f ms\n", cycles_to_ms(cyc_total_end - cyc_total_start));
        printf("[PW TIMING] params  : %.3f ms\n", cycles_to_ms(cyc_param));
        printf("[PW TIMING] weights : %.3f ms\n", cycles_to_ms(cyc_weight));
        printf("[PW TIMING] pack    : %.3f ms\n", cycles_to_ms(cyc_pack));
        printf("[PW TIMING] dma/run : %.3f ms\n", cycles_to_ms(cyc_dma));
        printf("[PW TIMING] store   : %.3f ms\n", cycles_to_ms(cyc_store));
    }
                             ...
                             const file_blob_t *weights_blob)
{
    ...
    const int in_pixels = d->H * d->W;
    const int out_pixels = outs.H * outs.W;
    int8_t w9[9];
...
int cnn_run_full_network(const runner_cfg_t *cfg)
{
    ...
    if (load_file_into_buffer_sd("0:/INPUTS/INPUT_~1.BIN", buf_cur, MAX_TENSOR_BYTES, &input_bytes) != 0) {
        if (load_file_into_buffer_sd("0:/INPUTS/INPUT_0.BIN", buf_cur, MAX_TENSOR_BYTES, &input_bytes) != 0) {
    ...
    Xil_DCacheFlushRange((UINTPTR)buf_cur, input_bytes);
    ...
    for (int i = 0; i < NUM_LAYERS; i++) {
        ...
        if (cfg && cfg->compare_layers) {
            ...
            if (compare_tensor_vs_ref(buf_next, out_bytes, ref_path, label, cfg) > 0 &&
                cfg->stop_on_fail) {
                free_blob(&instr_blob);
                free_blob(&params_blob);
                free_blob(&weights_blob);
...

        {
            ...
            if (ad) {
                ...
                if (cfg && cfg->compare_adds) {
                    ...
                    if (compare_tensor_vs_ref(buf_add, n, ref_path, label, cfg) > 0 &&
                        cfg->stop_on_fail) {
                        free_blob(&instr_blob);
                        free_blob(&params_blob);
                        free_blob(&weights_blob);
