...
module tb_dw_plane_run_axis_auto_fixed;
  ...
  // ------------------------------------------------------------------
  // DMA-realism stress knobs. The real board feeds the core from an
  // AXI-DMA (bursty s_axis with tvalid gaps) and drains via S2MM
  // (m_axis backpressure). The original TB drove s_axis continuously
  // and held tready=1, so it never exercised stalls. Set these to 1 to
  // reproduce board-like handshake timing; 0 restores the clean TB.
  // ------------------------------------------------------------------
  localparam bit STRESS_IN_GAPS = 1'b1;   // inject s_axis_tvalid gaps
  localparam bit STRESS_OUT_BP  = 1'b1;   // inject m_axis_tready backpressure

  // Output ready: optionally deassert to mimic S2MM backpressure (~75% ready)
  initial m_axis_tready = 1'b1;
  always @(posedge clk) begin
    if (!rst_n)             m_axis_tready <= 1'b1;
    else if (STRESS_OUT_BP) m_axis_tready <= (($random % 4) != 0);
  ...
  initial begin : stim_thread
    ...
    for (row_idx = 0; row_idx < inH; row_idx = row_idx + 1) begin
      ...
      for (col_word = 0; col_word < in_words_per_row; col_word = col_word + 1) begin
        ...
        // Mimic bursty DMA: drop tvalid for a couple of cycles periodically.
        if (STRESS_IN_GAPS && ((sent_total % 5) == 0)) begin
          s_axis_tvalid <= 1'b0;
