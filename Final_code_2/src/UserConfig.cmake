...
#
# 2026-07-30: enable NEON. The app was building with -mfpu=vfpv3 (inherited
# from the platform), so GCC emitted NO NEON at all and did not auto-vectorize.
# These are APPENDED to CMAKE_C_FLAGS, and the last -mfpu on the command line
# wins, so this overrides the platform default.
#   -mfpu=neon        Cortex-A9 media engine. ABI-compatible with the vfpv3
#                     BSP (same hard-float VFP register set; NEON is a superset).
#   -ftree-vectorize  not implied by -O3 for this target.
# Deliberately NOT adding -ffast-math: NEON single-precision on Cortex-A9 is
# not fully IEEE-754 (flush-to-zero denormals), and GCC will only auto-vectorize
# FP loops with fast-math. Integer loops vectorize without it, which is all this
# workload needs, so this stays numerically safe.
#
# Expect a MODEST gain: the dominant packing cost was the per-byte inner loop
# in pack_input_group_major (fixed separately), and what remains is an
# 8-byte-granular block scatter that is memory-bound, not ALU-bound.
set(USER_COMPILE_OTHER_FLAGS "-mfpu=neon -ftree-vectorize")
